...
const TUNE_PAWNS_PROTECTED_PASSER: bool = true;
...
pub struct Trace {
    ...
    pub pawns_protected_passer: [i8; 2],
...
impl From<Trace> for CompactTrace {
    fn from(t: Trace) -> Self {
        ...
        if TUNE_PAWNS_PROTECTED_PASSER {
            linear.push(t.pawns_protected_passer[1] - t.pawns_protected_passer[0]);
        }
...
impl Parameters {
    pub fn print_weights(&self) {
        ...
        if TUNE_PAWNS_PROTECTED_PASSER {
            print_single(self.linear[i], "PROTECTED_PASSED_PAWN");
            i += 1;
        }
...
impl Default for Parameters {
    fn default() -> Parameters {
        ...
        if TUNE_PAWNS_PROTECTED_PASSER {
            linear.push((
                mg(PROTECTED_PASSED_PAWN) as f32,
                eg(PROTECTED_PASSED_PAWN) as f32,
            ));
        }
