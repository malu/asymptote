...
const TUNE_PAWNS_ROOK_BEHIND_PASSER: bool = true;
...
pub struct Trace {
    ...
    pub pawns_rook_behind_passer: [i8; 2],
...
impl From<Trace> for CompactTrace {
    fn from(t: Trace) -> Self {
        ...
        if TUNE_PAWNS_ROOK_BEHIND_PASSER {
            linear.push(t.pawns_rook_behind_passer[1] - t.pawns_rook_behind_passer[0]);
        }
...
impl Parameters {
    pub fn print_weights(&self) {
        ...
        if TUNE_PAWNS_ROOK_BEHIND_PASSER {
            print_single(self.linear[i], "ROOK_BEHIND_PASSED_PAWN");
            i += 1;
        }
...
impl Default for Parameters {
    fn default() -> Parameters {
        ...
        if TUNE_PAWNS_ROOK_BEHIND_PASSER {
            linear.push((
                mg(ROOK_BEHIND_PASSED_PAWN) as f32,
                eg(ROOK_BEHIND_PASSED_PAWN) as f32,
            ));
        }
