...
const TUNE_PAWNS_ISLANDS: bool = true;
...
impl From<Trace> for CompactTrace {
    fn from(t: Trace) -> Self {
        ...
        if TUNE_PAWNS_ISLANDS {
            linear.push(t.pawns_islands[1] - t.pawns_islands[0]);
        }
...
impl Parameters {
    pub fn print_weights(&self) {
        ...
        if TUNE_PAWNS_ISLANDS {
            print_single(self.linear[i], "PAWN_ISLAND");
            i += 1;
        }
...
impl Default for Parameters {
    fn default() -> Parameters {
        ...
        if TUNE_PAWNS_ISLANDS {
            linear.push((mg(PAWN_ISLAND) as f32, eg(PAWN_ISLAND) as f32));
        }
